 ...
 *
 * Copyright 2015-2016 Matthias Reichl <hias@horus.com>
 *
...
#define rpi_dbg(dev, format, arg...) \
do { \
	if (debug & 1) \
		dev_printk(KERN_DEBUG, dev, format, ##arg); \
} while (0)

#define rpi_trace(dev, format, arg...) \
do { \
	if (debug & 2) \
		dev_printk(KERN_DEBUG, dev, format, ##arg); \
} while (0)
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	rpi_trace(card->dev, "spdif_rx event %d\n", event);
	...
	enum snd_soc_bias_level level)
{
	...
	rpi_trace(card->dev, "change bias level from %d to %d, sync=%d\n",
	...
	enum snd_soc_bias_level level)
{
	...
	rpi_trace(card->dev, "post change bias level to %d, sync=%d\n",
		level, priv->sync_path_enable);
	...
	struct snd_pcm_hw_params *params)
{
	...
	rpi_dbg(card->dev, "hw_params: setting to rate %d\n", rate);
...
static int rpi_cirrus_init_wm5102(struct snd_soc_pcm_runtime *rtd)
{
	...
	rpi_dbg(rtd->card->dev, "rpi_cirrus_init_wm5102\n");
...
static int rpi_cirrus_init_wm8804(struct snd_soc_pcm_runtime *rtd)
{
	...
	rpi_dbg(rtd->card->dev, "rpi_cirrus_init_wm8804\n");
...
static int rpi_cirrus_late_probe(struct snd_soc_card *card)
{
	...
	rpi_dbg(card->dev, "rpi_cirrus_late_probe\n");
	rpi_dbg(card->dev, "iec958_bits: %02x %02x %02x %02x\n",
